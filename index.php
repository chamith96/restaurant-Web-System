    ...
    if(isset($_POST['email']) && isset($_POST['password'])){
        ...
        if(!empty($email) && !empty($password)){
            ...
            $query ="SELECT email FROM user WHERE email='$email' and password='$md5_password'";
                ...
                echo "<script>window.onload = function(){alert('You entered username or password is wrong.');}</script>";
                    }
                }

        }else{
            echo "<script>window.onload = function(){alert('Fill Password or Email.');}</script>";
        }
    }
